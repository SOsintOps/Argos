...
zenity --info --text="The next window asks you to select a video file" --title="Video Utilities" 2> >(grep -v 'GtkDialog' >&2)
sleep 1
...
opt2="Convert to mp4"
opt3="Extract frames"
opt4="Compress video (low activity)"
opt5="Compress video (high activity)"
opt6="Extract audio (mp3)"
opt7="Rotate video"
    ...
    case $ffmpeg in
        ...
        "$opt2")
            "$FFMPEG_BIN" -i "$ffmpeg_file" -vcodec mpeg4 "$HOME/Videos/${timestamp}.mp4" \
                | zenity --progress --pulsate --no-cancel --auto-close --title="ffmpeg" --text="Converting to mp4..." 2> >(grep -v 'GtkDialog' >&2)
        ...
        "$opt3")
            ...
            "$FFMPEG_BIN" -y -i "$ffmpeg_file" -an -r 10 "$HOME/Videos/${timestamp}-frames/img%03d.bmp" \
                | zenity --progress --pulsate --no-cancel --auto-close --title="ffmpeg" --text="Extracting frames..." 2> >(grep -v 'GtkDialog' >&2)
        ...
        "$opt4")
            "$FFMPEG_BIN" -i "$ffmpeg_file" -vf "select=gt(scene\,0.003),setpts=N/(25*TB)" "$HOME/Videos/${timestamp}-low.mp4" \
                | zenity --progress --pulsate --no-cancel --auto-close --title="ffmpeg" --text="Compressing (low activity)..." 2> >(grep -v 'GtkDialog' >&2)
            xdg-open "$HOME/Videos/" >/dev/null 2>&1
            ;;
        "$opt5")
            "$FFMPEG_BIN" -i "$ffmpeg_file" -vf "select=gt(scene\,0.005),setpts=N/(25*TB)" "$HOME/Videos/${timestamp}-high.mp4" \
                | zenity --progress --pulsate --no-cancel --auto-close --title="ffmpeg" --text="Compressing (high activity)..." 2> >(grep -v 'GtkDialog' >&2)
            xdg-open "$HOME/Videos/" >/dev/null 2>&1
            ;;
        "$opt6")
            "$FFMPEG_BIN" -i "$ffmpeg_file" -vn -ac 2 -ar 44100 -ab 320k -f mp3 "$HOME/Videos/${timestamp}.mp3" \
                | zenity --progress --pulsate --no-cancel --auto-close --title="ffmpeg" --text="Extracting audio..." 2> >(grep -v 'GtkDialog' >&2)
            xdg-open "$HOME/Videos/" >/dev/null 2>&1
            ;;
        "$opt7")
            "$FFMPEG_BIN" -i "$ffmpeg_file" -vf transpose=0 "$HOME/Videos/${timestamp}-rotated.mp4" \
                | zenity --progress --pulsate --no-cancel --auto-close --title="ffmpeg" --text="Rotating video..." 2> >(grep -v 'GtkDialog' >&2)
            xdg-open "$HOME/Videos/" >/dev/null 2>&1
            ;;
    esac
